...
function userJoin(id, username, room) {
  ...
  if (name.includes("  "))
    return { error: "Username cannot contain consecutive spaces" };
  ...
  if (usernames.has(key)) {
    ...
    if (existingUser && existingUser.id !== id) {
      ...
      console.log(
        `[DUPLICATE REJECTED] Username "${name}" (key: "${key}") is already taken by socket ${existingUser.id}, rejecting socket ${id}`
      );
      return {
        error: `Username "${name}" is already taken. Please choose a different username.`,
      };
    }
  ...
  usernames.add(key);
  console.log(
    `[USER JOINED] Username: "${name}" (key: "${key}"), Socket: ${id}, Total users: ${users.length}, Usernames in Set: ${usernames.size}`
  );
  return user;
}
